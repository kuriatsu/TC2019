...
pcl::PointCloud<pcl::PointXYZI> buffer_pcl_;
...
void laser_callback(const sensor_msgs::LaserScan::ConstPtr& msg)
{
	...
	for (int i=0;i<in_scan.points.size();i++)
  {
		...
		point.z = in_scan.points[i].z;
		point.intensity = 255;
		if (fabs(point.y/point.x) < 1.7321) //Tan 60
		{
			buffer_pcl_.push_back(point);
		}
  }
}
...
GroundFilter::GroundFilter() : node_handle_("~")
{
	...
	safe_sub_ = node_handle_.subscribe("/scan", 2, laser_callback); // subscribe to urg node's topic
	points_node_sub_ = node_handle_.subscribe(point_topic_, 10000, &GroundFilter::VelodyneCallback, this);
...
void GroundFilter::VelodyneCallback(const pcl::PointCloud<velodyne_pointcloud::PointXYZIR>::ConstPtr &in_cloud_msg)
{
	...
	velodyne_pointcloud::PointXYZIR point;
	for (size_t i = 0; i < buffer_pcl_.points.size(); i++)
	{
		point.x = buffer_pcl_.points[i].x;
		point.y = buffer_pcl_.points[i].y;
		point.z = buffer_pcl_.points[i].z;
		point.intensity = buffer_pcl_.points[i].intensity;
		point.ring = 0;
