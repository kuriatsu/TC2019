...
int main(int argc, char *argv[]){
  ...
  tf::Transform tf_cam2velodyne;
  tf_cam2velodyne.setOrigin(tf::Vector3(0,0,0));
  tf::Quaternion q;
  q.setRPY(-M_PI/2,0,-M_PI/2);
  tf_cam2velodyne.setRotation(q);
  ...
  for(std::vector<std::pair<int, tf::Transform> >::iterator it=orb_results.begin(); it!=orb_results.end(); it++){
	...
	sqrt((key_map[it->first].first.getOrigin().x() -
      ...
      tf::Transform scaled_rel=orb_rel*tf_cam2velodyne;
      scaled_rel.setOrigin(orb_rel.getOrigin()*scale);

      tf::Transform position=key_map[it->first].second*scaled_rel;
      
      printf("%f %f %f %f %f %f %f %f\n",scale,scaled_rel.getOrigin().x(),scaled_rel.getOrigin().y(),scaled_rel.getOrigin().z(),position.getOrigin().x(),position.getOrigin().y(),position.getOrigin().z()
	     );
    }
  }
  return(0);
