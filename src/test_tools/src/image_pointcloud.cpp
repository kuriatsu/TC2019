...
double getAngle(double x, double y) {
   ...
   double lengthYaxis = sqrt(x1 * x1 + y1 * y1);
   double lengthOther = sqrt(x*x + y*y);
   double dot = (x1 * x) + (y1 * y);
   double angle = acos(dot  / (lengthYaxis * lengthOther)) * (180.0 / PI);
   ...
   */
   ...
   /*
   if (angle < 0)    
       std::cout << "angle to x,y=" << x << ", " << y << "=" << angle << endl;
   */
...
void callback(const pcl::PointCloud<velodyne_pointcloud::PointXYZIR>::ConstPtr& msg){
  //std::cout << "pointcloud callback called!" << std::endl;
  ...
  {   
    ...
    if (center_ready == false ) //|| right_ready == false)
      return;
    printf("hoge\n");
    //i found that you need a separate CvBridge for each image
    //imageCenter = bridgeCenter.imgMsgToCv(imgCenter, "bgr8");
    ros::Time veltime;    
    pcl_conversions::fromPCL(msg->header.stamp,veltime);    
    char filename_velodyne[100];//,filename_img[100];    
    //sprintf(filename_velodyne,"/tmp/%.2f_velodyne.txt",veltime.toSec());
    //sprintf(filename_img,"/tmp/%.2f_camera.bmp",imgCenter->header.stamp.toSec());
  
    cv::Mat cppMat =cv_bridge::toCvShare(imgCenter, "bgr8")->image;
    imageCenter=cppMat;
    printf("image copy\n");
    //printf("%f %f %f\n",veltime.toSec(),ros::Time::now().toSec(),veltime.toSec()-ros::Time::now().toSec());
    //veltime=ros::Time::now()+ros::Duration(0.1);
    
    
  
    //  listenerCenter_.waitForTransform("velodyne", "camera",   ros::Time::now(), ros::Duration(1));
    listenerCenter_.lookupTransform( "camera",   "velodyne", ros::Time(0), centerTransform);  
    printf("get transform\n");  
    //FILE* fp;
    //fp=fopen(filename_velodyne,"w");
